...
try
    ...
    error('TNT file could not be processed')
end
...
for ii = 1:numel(storeNames)
    ...
    if isfield(headerStruct.stores.(varName), 'ts')
        filterInd = find(headerStruct.stores.(varName).ts >= validTimeRange(1) & headerStruct.stores.(varName).ts < validTimeRange(2));
        if ~isempty(filterInd)
            ...
            if strcmpi(headerStruct.stores.(varName).typeStr, 'streams')
                ...
                if filterInd(1)-nchan > 0
                    filterInd = [-(double(nchan:-1:1)) + filterInd(1) filterInd];
                end
                temp = headerStruct.stores.(varName).ts(filterInd);
                headerStruct.stores.(varName).startTime = temp(1);
                headerStruct.stores.(varName) = rmfield(headerStruct.stores.(varName), 'ts');
            else
                headerStruct.stores.(varName).ts = headerStruct.stores.(varName).ts(filterInd);
            end
            if isfield(headerStruct.stores.(varName), 'chan')
                if numel(headerStruct.stores.(varName).chan) > 1
                    headerStruct.stores.(varName).chan = headerStruct.stores.(varName).chan(filterInd);
                end
            end
            if isfield(headerStruct.stores.(varName), 'sortcode')
                headerStruct.stores.(varName).sortcode = headerStruct.stores.(varName).sortcode(filterInd);
            end
            if isfield(headerStruct.stores.(varName), 'data')
        ...
        % handle epoc events
        filterInd = find(headerStruct.stores.(varName).onset >= validTimeRange(1) & headerStruct.stores.(varName).onset < validTimeRange(2));
        if ~isempty(filterInd)
            ...
            % fix time ranges
            if headerStruct.stores.(varName).offset(1) < headerStruct.stores.(varName).onset(1)
                if headerStruct.stores.(varName).onset(1) > validTimeRange(1)
                    headerStruct.stores.(varName).onset = [validTimeRange(1) headerStruct.stores.(varName).onset];
                end
            end
            if headerStruct.stores.(varName).offset(end) > validTimeRange(2)
                headerStruct.stores.(varName).offset(end) = validTimeRange(2);
            end
        end
    end
    ...
    if lia
        ...
        validInd = bitand(ts >= validTimeRange(1), ts < validTimeRange(2));
...
for ii = 1:numel(storeNames)
    ...
    elseif isequal(currentTypeStr, 'streams')
        ...
        if useSEVs
            ...
            if CHANNEL == 0
                d = SEV2mat(BLOCK_PATH, 'EVENTNAME', currentName, 'VERBOSE', 0);
            else
                d = SEV2mat(BLOCK_PATH, 'EVENTNAME', currentName, 'CHANNEL', CHANNEL, 'VERBOSE', 0);
            end
            ...
            if CHANNEL > 0
                ...
                allOffsets = double(headerStruct.stores.(currentName).data(validInd));
                if numel(headerStruct.stores.(currentName).chan) > 0
                    headerStruct.stores.(currentName).chan = headerStruct.stores.(currentName).chan(validInd);
                end
                nchan = 1;
                chanIndex = 1;
                ...
                validInd = 1:numel(headerStruct.stores.(currentName).chan);
            end
            
            % preallocate data array
            npts = (currentSize-10) * 4/sz;
            headerStruct.stores.(currentName).data = zeros(nchan, npts*numel(validInd)/nchan, fmt);
            
            % try to optimally read data from disk in bigger chunks
            maxReadSize = 10000000;
            iter = min(8192, size(allOffsets,2)-1);
            arr = 1:iter:size(allOffsets,2);
            markers = allOffsets(arr);
            ...
            for f = 1:numel(arr)
                ...
                for jj = 1:numel(relativeOffsets)
                    if numel(headerStruct.stores.(currentName).chan) > 1
                        chan = headerStruct.stores.(currentName).chan(channelOffset);
                    else
                        chan = headerStruct.stores.(currentName).chan;
                    end
                    channelOffset = channelOffset + 1;
                    if CHANNEL > 0
                        headerStruct.stores.(currentName).data(1, chanIndex:(chanIndex + npts - 1)) = tevData(ind(jj,:))';
                        chanIndex = chanIndex + npts;
                    else
                        headerStruct.stores.(currentName).data(chan, chanIndex(chan):chanIndex(chan) + npts - 1) = tevData(ind(jj,:));
        ...
        minSample = max(ceil((validTimeRange(1)-headerStruct.stores.(currentName).startTime)*headerStruct.stores.(currentName).fs),0)+1;
        maxSample = min(max(ceil((validTimeRange(2)-headerStruct.stores.(currentName).startTime)*headerStruct.stores.(currentName).fs),0)+1, size(headerStruct.stores.(currentName).data, 2)) ;
        headerStruct.stores.(currentName).data = headerStruct.stores.(currentName).data(:,minSample:maxSample);
        headerStruct.stores.(currentName).startTime = headerStruct.stores.(currentName).startTime + (minSample-1) / headerStruct.stores.(currentName).fs;
    end
