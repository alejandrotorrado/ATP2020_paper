                ...
                for yy = 1:nChunks;
                    ...
                    if ~isempty(subsweeps{yy})
                        ...
                        if strcmp(myname,'marmoset')
                            load('C:\Users\khengen.USERS\Google_Drive\Matlab_scripts_11_01_2012\Cell_Quality\RANDOMFOREST_TRAINED\rForest_Mdl_KH72_73_75_SHANK01_AT14_trained.mat')
                        elseif strcmp(myname, 'coypu')
                            load('C:\Users\khengen\Google_Drive\Matlab_scripts_11_01_2012\Cell_Quality\RANDOMFOREST_TRAINED\rForest_Mdl_KH72_73_75_SHANK01_AT14_trained.mat')
                        end
                        ...
                        p_out = []; trimvec = [];
                        [block{yy}, p_out, trimvec] = alignandcluster(subsweeps{yy},...
                            thischan,yy,houredges,subidx{yy},scrubbedidx,times,0,97,...
                            Mdl,minrng,swprng,nclust);
                        % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                        ...
                        if sum(trimvec) == 0
                            ...
                            nclust_trim = nclust + 1;
                            trimmers = find(trimvec == 1);
                            tflag = 0;
                            
                            % build an idx list
                            idx_tmp = [];
                            for jj = 1:size(block{yy}.clust,2);
                                idx_tmp = [idx_tmp; block{yy}.clust(jj).idx];
                            end
                            idx_tmp = sortrows(idx_tmp);
                        end
                        ...
                        while tflag < 2 % trim twice (change this number to change number of passes)
                            ...
                            for vv = 1:sum(trimvec)
                                clust = []; dist = [];
                                clust   = block{yy}.clust(trimmers(vv));
                            ...
                            idx_tmp(clip)           = [];
                            sweepsCLIPPED           = subsweeps{yy}(idx_tmp,:);
                            ...
                            [block{yy}, p_out, trimvec] = alignandcluster(sweepsCLIPPED,...
                                thischan,yy,houredges,subidx{yy},idx_tmp,times,0,97,...
                                Mdl,minrng,swprng,nclust_trim);
...
%                         % coded (see top of script).
                        ...
                        p_save{yy}(end+1:end+size(p_out,1),:) = p_out;
                ...
                if strcmp(myname,'marmoset')
                    load('C:\Users\khengen.USERS\Google_Drive\Matlab_scripts_11_01_2012\Cell_Quality\RANDOMFOREST_TRAINED\rForest_Mdl_KH72_73_75_trained.mat')
                elseif strcmp(myname, 'coypu')
                    load('C:\Users\khengen\Google_Drive\Matlab_scripts_11_01_2012\Cell_Quality\RANDOMFOREST_TRAINED\rForest_Mdl_KH72_73_75_trained.mat')
                end
                ...
                p_out = []; trimvec = [];
                [block{1}, p_out, trimvec] = alignandcluster(sweeps,thischan,1,houredges,subidx,scrubbedidx,times,0,97,Mdl,minrng,swprng,nclust);
        % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                % Check for a trim flag and set up for further (in)action:
                trimstat = 0;
                if sum(trimvec) == 0
                    ...
                    % build an idx list
                    idx_tmp = [];
                    for jj = 1:size(block{1}.clust,2);
                        idx_tmp = [idx_tmp; block{1}.clust(jj).idx];
                    end
                    idx_tmp = sortrows(idx_tmp);
                end
                ...
                while tflag < 2 % trim twice (change this number to change number of passes)
                    ...
                    clip = []; killz = []; % Assemble all of the points to be trimmed:
                        ...
                        for vv = 1:sum(trimvec)
                            clust = []; dist = [];
                            clust   = block{1}.clust(trimmers(vv));
                            dist    = clust.dists./max(clust.dists);
                            tmpkillz   = find(dist > thresh);
                            killz = clust.idx(tmpkillz);
                            clip    = [clip; killz];
                        ...
                        [~,idx_tmp2,~] = intersect(scrubbedidx,idx_tmp);
                        ...
                        fprintf('Channel %u is being trimmed!\n',thischan);
                        [block{1}, p_out, trimvec] = alignandcluster(sweepsCLIPPED,...
                            thischan,1,houredges,subidx,idx_tmp,times,0,97,...
                            Mdl,minrng,swprng,nclust_trim);
                ...
                p_save(end+1:end+size(p_out,1),:) = p_out;
...
if nChunks == 0
    CELL = formatAutoCELL(CELL_BACKUP,nChunks,0);
...
elseif nChunks > 0    
    CHUNKS_init = formatAutoCELL(CELL_BACKUP,nChunks,0);
