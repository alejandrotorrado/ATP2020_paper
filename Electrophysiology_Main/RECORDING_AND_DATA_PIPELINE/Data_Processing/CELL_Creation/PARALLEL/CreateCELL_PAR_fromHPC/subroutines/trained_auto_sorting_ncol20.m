...
% source. This script will extract 19 features from the cluster and its
% constitutent waveforms and use these to make a decision. The decision
% making process calls upon a supervised learning model (machine learning)
% that, when built, aimed to take hundreds of manually coded clusters and
% learn the strongest set of discriminatory rules based on that input.
% Here, the same rules are applied to novel inputs (the clusters). This
% training dataset can be updated with new information to increase accuracy
% if anything is systematically mislabeled.
%
% KBH 2/2017
...
for ee = 1:size(ogdat.clust,2)
    ...
    if ~isempty(h)
        if sum(h)<1000
            ...
            p(count,1:2) = gamfit(1:out_to,[],[],h(1:out_to)');
            ...
            % of recording time) and then average the hourly means together to
            frac = 0.10; % fraction of experiment to exclude on either side
            % get an overall value
