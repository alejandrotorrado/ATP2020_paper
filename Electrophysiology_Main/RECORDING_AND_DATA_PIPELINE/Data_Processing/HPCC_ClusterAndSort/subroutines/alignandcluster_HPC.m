...
% interpolate and re-center the spikes if asked to
if doInterp
    ...
    % otherwise just align
	[~,minidx2] = min(sweepdat(:,nminrng),[],2);
	minidx2    = minidx2 + nminrng(1) - 1;
    
    % align using alignrows
    sweepdat        = alignrows(sweepdat, minidx2, nswprng, size(sweepdat,2)-1);
end
...
for gg = 1:size(block.clust,2);
    ...
    % find average WF and its st. dev. for each cluster
    block.clust(gg).meantrace   = mean(sweepdat(block.clust(gg).idx,:));
    block.clust(gg).tracestdev  = std( sweepdat( block.clust(gg).idx,: ) );
    
    % update idx to correspond to the original spikes files. This has to be
    % done because of the subsetting and because of trimming.
    if ~isempty(subidx)
