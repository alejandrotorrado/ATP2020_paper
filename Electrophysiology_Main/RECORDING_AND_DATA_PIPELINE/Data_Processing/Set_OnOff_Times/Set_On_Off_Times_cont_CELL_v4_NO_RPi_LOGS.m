%function Set_On_Off_Times_cont_CELL_v4_RPi_LogsUpdate
%
...
if ismac
    if exist('/Users/atorrado/','dir')
        lDir = '/Users/atorrado/Desktop/MLS_DATA';
    elseif exist('/Users/brianlane/','dir')
        lDir = uigetdir('/Users/brianlane/Desktop','Pick directory where expt start file is.');
    end
elseif ispc
    lDir = uigetdir('C:\','Pick directory where expt start file is.');
end
...
if ~exist('CELL','var')
    ...
    load([lDir lFile]);
    if exist('CHUNKS','var') && ~exist('CELL','var')
        CELL = CHUNKS(1).CELL;
        clear CHUNKS
    end
else
    fprintf('\n\nFound CELL variable (n=%u).\n\n',length(CELL));
end
...
if ~isfield(CELL,'EXPTSTART')
    %try to find expt_START file
    if exist('lDir','var')
        expt_start_file = dir([lDir filesep animal '_expt_START.mat']);
    else
        lDir = uigetdir(cd,'Point me to folder containing Expt Start file');
        expt_start_file = dir([lDir filesep animal '_expt_START.mat']);
    end
    if numel(expt_start_file) == 1
        expt_st = expt_start_file(1).name;
        fprintf('Found experiment start file: %s\nLoading it...\n',expt_st);
        expt_start_load = load([lDir filesep expt_st]);
        for cx = 1:length(CELL)
            CELL(cx).EXPTSTART = expt_start_load.tINFO;
        end
    else
...
while ee <= size(CELL,2)
    ...
    if CELL(ee).quality < 4
        ...
        if any(CELL(ee).channel == valchans)
            ...
            A = histc(CELL(ee).time, 0 : 300 : mT );
            x = 1: 300 : mT ;
            B = A/300;
            if size(B,1) ~= size(x,2)
                B(end) = [];
            end
            ...
            ln2 = line([mT-3600*6 mT-3600*6], [0 ceil(max(B))]);
            set(ln2,'linestyle','--','color',[0.2 0.2 0.2]);
            
            % Plot the firing rate trace
            set(h,'units','normalized','position',[0.01 0.25 0.8 0.7]);
            plot(x, B)
...
if exist('lFile','var')
    ...
    nFile = [animal '_MASTER_CELL_onOff.mat'];
end
disp(['Saving updated CELL file to ' lDir filesep nFile]);
drawnow;
save([lDir filesep nFile],'CELL','-v7.3');
clc
