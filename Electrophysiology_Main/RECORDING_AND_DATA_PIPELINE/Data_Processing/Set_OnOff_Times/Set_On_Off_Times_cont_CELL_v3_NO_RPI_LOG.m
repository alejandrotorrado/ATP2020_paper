...
% of 2016 by KBH to run faster an0d to incorporate on and off times
...
for ee = 1:length(CELL);
    ...
    CELL(ee).neg_pos_time   = calc_negpostime(swf);
    CELL(ee).halfwidth      = calc_halfwidth(swf);
    % tail slope
...
%     CELL(ee).tailSlope = slope(1);
    CELL(ee).tailSlope = calc_tailslope(swf);
end
% - - - - - - - - - - - - - - - - - - -
...
while ee <= size(CELL,2);
    ...
    if CELL(ee).quality <= 3;
        ...
        if any(CELL(ee).channel == valchans);
            ...
            set(h,'units','normalized','position',[0.01 0.05 0.98 0.85]);
            plot(x, B)
            set(gca,'xlim',[0 nbins*12*3600],'ylim',[0 max(ceil(max(B)),1)]);
            ...
            elseif size(tmpWF,2) == 59
                WFx     = linspace(edgesD(1)*3600,edgesD(1)*3600 + 3600*40,59);
            end
