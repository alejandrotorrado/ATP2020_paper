...
for ii = start_here:last_block % set start back to 1
    ...
    split_1 = regexp(one_file,'Crickets_BJL-','split');
    split_1 = split_1{end};
    split_2 = regexp(split_1,'_RAWX','split');
    split_2 = split_2{1};
    
    if isempty(regexp(last_TSQ_dir,split_2,'ONCE'))
        all_tsqdirs = dir([COYPU_DRIVE filesep 'Crickets_BJL-*']);
        ...
        if isempty(TSQstamp) && not (tsqtemp) && ii > 1
            ...
            TSQstamp = unixtime(datevec(TimezoneConvert(datetime(unixtime(tsqtemp)),'UTC','America/New_York'))); % BJL Note: Jan 24 2020, Error, possibly replaced datestr with datetime to fix.  Not 100% sure this will work.
