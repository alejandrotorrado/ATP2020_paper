function [spiketimes, sweeps] = signal2spikes_sdthresh_upsample_LGN(signal, signal_starttime, signal_sampleinterval, sdthresh, numPts, upfactor)
...
% KBH beta testing spline fit of 5x interpolation (upsampling)  - - -

%oldsignal   = highlowpassfilter(signal);
if isempty(signal)
    spiketimes = [];
    sweeps = [];
   warning('FOUND AN EMPTY CHANNEL. No signal is extracted. Processing will skip this and move on.');  
    ...
    % seems to add ~15 sec of processing per channel per hour
    % - - - - - - - - - - - - - -
    
    %%% FUNKYFRESH NOISE FILTER (TM)!!!!
    %[funkindex,funkback] = funkyfresh(signal,funkrecord);
    
    
    %this is dynamic threshholding? signal is loaded in 60 sec chunks...
    ...
    if ~isempty(spiketimes);
        ...
        [posrow,~]          = find(sweeps>7e-4 | sweeps <-7e-4); % Filter out huge traces;
