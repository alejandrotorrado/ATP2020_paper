% SPIKE EXTRACTION MAIN SCRIPT - September 2016, ATP
%
% *** IMPORTANT ***
...
% Custom function to clear memory and workspace variables
clearIDE

% **************************** IMPORTANT NOTE *****************************
% This script waits until block 100 has been backed up (by
% CopyBackup_MainScript.m) before starting to extract spikes on block 99.
% This is to make sure that all of  block 99 has been copied before spike
% extraction is done. This means that for the last block, the program will
% wait indefinitely. To do spike extraction on the last block, follow these
% steps:
%
%   1 - MAKE ABSOLUTELY SURE THAT THE LAST BLOCK HAS BEEN BACKED UP
%   2 - Make sure again. When you are certain, interrupt this script from
%       running (it should be stuck in waiting mode).
%   3 - Change both the start_here and last_block variables to the last
%       block number
%   4 - Go down to line 195 and run the code from the highlighted section
%       onwards
%
% *************************************************************************
% Choose starting block and last block
start_here = 1;
last_block = 300;
...
% This was added when we started doing LGN experiments
...
% This is relevant because the channel used for the experiment timer may
% change depending on how the rig is set up
...
COYPU_DRIVE  = uigetdir('Y:\TDT_DATA_TANKS\','Where is the TDT data tank on Coypu that has the tsq files?'); % Rig PC tsq file storage
...
% initialize empty variables for the TSQ timestamp and directory
TSQstamp = [];
last_TSQ_dir = '';
% gate to tell code whether TSQ timestamp is GMT corrected. This should
% ALWAYS BE 0 when you start this code
gmt_corrected = 0; 

%% RUN FROM HERE FOR THE LAST BLOCK
% Loop through blocks
for ii = start_here:last_block
    
    % find current block directories
    ...
    % recording (and first after a crash). Use this to find the experiment
    % start time:
    tsqtemp = [];
    fprintf('\nLooking for TSQ timestamp...\n');
    % Index the current block directory
    ...
    % Get the first SEV file name
    fprintf('  Found files!\n\n');
    one_file = all_files(1).name;
    % The name has date and timestamp for the experiment. Use regexp to get
    % it
    split_1 = regexp(one_file,'Chronic-','split');
    split_1 = split_1{end};
    split_2 = regexp(split_1,'_RAWX','split');
    split_2 = split_2{1}; % this is it
    
    % check whether there is a TSQ directory with that name
    if isempty(regexp(last_TSQ_dir,split_2,'ONCE'))
        % if there isn't one, index all the TSQ directories and find the
        % new one
        ...
        % checkTSQ is a custom function that gets the experiment start time
        % stamp from the TSQ file
    ...
    thisrow = [];
    % main loop to extract spikes. There are several calls to subroutines
    % that then call each other. The tree looks like:
    % 
    % _makeSpikeFiles (or makeSpikeFiles_16ch or makeSpikeFiles_32ch)
    %   _spikesFromTDTRaw_Upsample
    %       _signal2spikes_sdthresh_upsample_LGN (or signal2spikes_sdthresh_upsample_LGN)
    for ee=1:animalNum
        if ee==1
        ...
        elseif ee==2
            ...
            if ~exist([DATADRIVE filesep subject2 filesep 'ALLSPIKES' filesep (subject2)...
                ...
                % This function creates both the .mat spike files and the
                % binary spike files. The latter are the really important
                % ones that are used for clustering and sorting.
                if tot_chans == 64
...
% 09/08/2016
%% This now includes backup of binary spike files
...
% This file ends up in each animal's data folder and contains a timestamp
% for the start of the experiment (start of block 1).
...
copyfile([svdir_2 filesep 'BinarySpikes' filesep '*.mat'],backup_bin_2);
t21e = toc(t21);
...
copyfile([svdir_2 filesep 'BinarySpikes' filesep '*.mat'],backup_bin_2a);
t22e = toc(t22);
