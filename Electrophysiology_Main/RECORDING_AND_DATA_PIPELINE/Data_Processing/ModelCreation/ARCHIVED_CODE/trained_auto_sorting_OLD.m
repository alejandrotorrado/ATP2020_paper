...
load('/Users/khengen/Desktop/PROFILESETUP/P_KH73_75.mat');
...
%   Column 13 is amplitude
%   Column 14 is 60 cycle noise measure
%   Column 15 is subjective quality
...
Mdl = TreeBagger(200,training,scoredclass,'OOBPrediction','On',...
    'Method','classification', 'CategoricalPredictors', catpre,'OOBPredictorImportance','On'); %'OOBPredictorImportance','On'
...
% NEED TO OPTIMIZE AND PLOT AND SCORE ETC THE TRAINING DATASET MORE...

% OR LOAD PRIOR MODEL:
% load('/Users/khengen/Google_Drive/Matlab_scripts_11_01_2012/Cell_Quality/RANDOMFOREST_TRAINED/rForest_Mdl_KH72_73_75_SHANK01_trained.mat');
% load('/Users/atorrado/Google_Drive/Matlab_scripts_11_01_2012/Cell_Quality/RANDOMFOREST_TRAINED/rForest_Mdl_KH72_73_75_SHANK01_AT14_trained.mat');
...
count = 0; p = []; scount = 0; pcount = 0; chcount = 0;
for ii =  1:32 % size(chanfiles,1)
    chanfilename = [sDir filesep 'channel_' num2str(ii) '.mat'];
    
    if exist(chanfilename,'file') == 2
        chcount = chcount +1;
        ...
        %         l1 = strfind(chanfiles(ii).name,'_');
        %         l2 = strfind(chanfiles(ii).name,'.mat');
        %         chan = str2double(chanfiles(ii).name(l1+1:l2-1));
        ...
        for ee = 1:size(chdat.clust,2)
            ...
            if sum(h)<1000
                ...
                %                 plot(RSUwin2,'g');
                %                 plot(FSwin2,'r');
                meanfr = numel(chdat.clust(ee).time)/(chdat.clust(ee).time(end)-chdat.clust(ee).time(1));
...
[zFile, zDir] = uiputfile(cd,'Save corrected autoscored output');
save([zDir zFile],'p');
