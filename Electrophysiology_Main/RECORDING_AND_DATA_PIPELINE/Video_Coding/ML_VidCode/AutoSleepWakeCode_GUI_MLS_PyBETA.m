...
function AutoSleepWakeCode_GUI_MLS_PyBETA
warning('off');
global myname
% set environment variable to include anaconda's python libraries.
% NOTE! This will only work on my laptop... will need to find a way to
% change this.
%setenv('PATH','/Applications/anaconda/bin:/anaconda/bin:/opt/local/bin:/opt/local/sbin:/opt/local/bin:/opt/local/sbin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/X11/bin');
if ismac
    ...
    function LoadMvmt(~,~)
        ...
        if isfield(temp,'DATA')
            ...
            if isempty(lfp_start)
                lfp_start = handles.D.LFPdata(2).startTime;
            end
...
%             keyboard;
            if abs(lfp_start-mvt_start) >= (4*3600 - 300) && ...
                    abs(lfp_end-mvt_end) >= (4*3600 - 300)
...
%             keyboard;
        ...
        if var(emgLtemp) > var(emgRtemp);
            ...
            if ~use_EMG_avg && have_both_EMGs
                ...
                emgavg = nanmean(padcat(emgLtemp, emgRtemp));
            end
            ...
            for ee = 1:size(bb,2)-1
                ...
                rs_emg(ee) = nanmean(handles.D.emgY( 1, bb(ee):bb(ee+1) ) );
            end
            keyboard;
            handles.D.rs_emg = rs_emg;
...
%             keyboard;
            emg_mean = nanmean( handles.D.rs_emg);
            emg_std  = nanstd( handles.D.rs_emg);
            emg_zscore = ( handles.D.rs_emg - emg_mean) ./ emg_std;
...
% keyboard;
        ...
        handles.D.feature_data = [deltapower, thetapower, dt_diff, rs_mvt_z];
        if handles.use_EMG
        ...
        if handles.D.block <= handles.D.X_Block
            ...
            [testdat, rawdat, block_features] = score_10sec_noMLS(handles.D.feature_data, handles.D.pX, sampling_rate);
            ...
            if handles.master_doTrain
                ...
                [pymdl_path,oob_err,savemdl_dir] = AutoVidCode_trainModel_PyWrap(handles.D.training_data,...
                    handles.D.animal,handles.D.ml_algorithm,RF_nTrees);
...
%             loaded_Mdl = AutoVidCode_loadModel(handles.D.animal,ml_algorithm);
            ...
            [testdat, rawdat, block_features] = auto_video_score_PyWrap(handles.D.feature_data, handles.D.trained_Mdl_path, ...
...
%                     delete(handles.EMGfig);
        ...
        if handles.use_EMG
            keyboard;
            handles.EMGfig = axes('Position',psd_axes_pos,'Color','none','Parent',handles.f);
            axes(handles.EMGfig);
            % NOTE 10/30/2019: in all of this, handle.D.emg_z used to be
            % handles.D.rs_emg
            %             emg_ylim = abs(mean(handles.D.emg_z)) + 0.5*std(handles.D.emg_z);
            %             emg_ylim = [
            %             yscale_emg = 'log';
            if min(handles.D.rs_emg) < 0
                emg_ylim_1 = mean(handles.D.rs_emg) - 5*std(handles.D.rs_emg);
                emg_ylim_2 = mean(handles.D.rs_emg) + 15*std(handles.D.rs_emg);
                yscale_emg = 'linear';
            else
                emg_ylim_1 = mean(handles.D.rs_emg) - 5*std(handles.D.rs_emg);
                emg_ylim_2 = mean(handles.D.rs_emg) + 15*std(handles.D.rs_emg);
...
%         plot_emg = plot(handles.D.pX, handles.D.emg_z,'-','linewidth',2,...
%             'color',[0 .45 .74]);
        hold off;
        ...
        if handles.use_EMG
            if handles.use_gamma
                plot_legend = legend([plot_theta,plot_delta,plot_gamma,plot_mvt,plot_emg],...
                    {'Theta','Delta','Gamma','Movement','EMG'},'fontsize',14);
            else
                plot_legend = legend([plot_theta,plot_delta,plot_mvt,plot_emg],...
                    {'Theta','Delta','Movement','EMG'},'fontsize',14);
            end
        else
             plot_legend = legend([plot_theta,plot_delta,plot_mvt],...
            {'Theta','Delta','Movement'},'fontsize',14);
        end
...
%         stmp = [stmp0(stmp0(:,2)>=rd(1,2) & stmp0(:,2)<=rd(end,2),:); [stmp0(end,1) rd(end,2)+10]];
        ...
        if ~handles.D.no_training && handles.master_doTrain
            if handles.D.block == 1
                ...
                if isfield(handles.D,'training_data')
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    function GoToBlock(~,~)
        ...
        if ~isfield(handles.D,'features') || length(handles.D.features) < (handles.D.block) || handles.D.prior
