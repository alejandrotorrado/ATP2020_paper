function [datout,datraw,feat_out] = score_10sec_noMLS_Quiet (data, tx, Fs)
...
%
%       NOTE: Organization of DATA = [deltapower, thetapower, deltatheta_difference, rs_mvt_z, quiet_z, emg_zscore];
%       NOTE: REM = 1, NREM = 2, ACTIVE = 4, QUIET = 5
% keyboard;


%%
nFeatures = 12; % hard-coded for now
bin_size = 10; % seconds
...
for ee = 1:size(edges,2)-1
    ...
    if isempty(quiet_data)
        quietmean = NaN;
    else
        quietmean = nanmean(quiet_data);
    end
