...
%     end
    ...
    % ATP ADDED THESE TWO IF STATEMENTS
    int_frame = mean(mean(frame_doub));
    if int_frame >= datosegm.int_thresh
        umbral = datosegm.umbral_LIGHT;
    elseif int_frame < datosegm.int_thresh
        umbral = datosegm.umbral_DARK;
    end
    if length(umbral) > 1
        umbral = umbral(1);
    end
...
%     frame_doub=frame_doub./mean(mean(frame_doub));
    % line above is original, line below is mine (ATP)
    
    if all(~isnan(frame_doub(:))) % Porque en algunos vídeos hay algún frame raro que es todo negro
        suma = suma + frame_doub;
        videomedio_cuentaframes=videomedio_cuentaframes+(frame_doub<umbral);
        c_frames=c_frames+1;
    end
