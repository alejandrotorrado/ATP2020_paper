...
function [datosegm,h]=panel_identitracking_ATP(datosegm)
...
% hold(h.colorbar,'on')
% h.lineaumbral=line(h.colorbar.Children,'xdata',[-100 100],'ydata',[1 1]*datosegm.umbral,'zdata',1,'Color',color_manchas,'LineWidth',2);
...
% ******************************** ATP ************************************ change code here
cogezona(h.push_roi,[],h);          % set ROI
calculavideomedio([],[],h);         % compute background
...
datos=guidata(h.fig);
% ************************************** ATP CHANGED THIS *****************
datos.datosegm=datosegm2videomedio(datos.datosegm,datos.datosegm.nBkgFrames,h); % change the 2nd argument from 100 to new number
...
% ************ ATP CHANGED THIS HERE **************************************
if isempty(datos.datosegm.roi)
    % only ask for roi if 
...
for c=1:length(hijos)
    if ~strcmpi(get(hijos(c),'Type'),'axes')
        if estado % Enable todo lo que corresponda
            ...
            if ~strcmpi(get(hijos(c),'Type'),'colorbar')
