...
%%% ****** ATP changed this *******
umbral_npixels_LIGHT=datosegm.umbral_npixels_LIGHT;
umbral_npixels_DARK=datosegm.umbral_npixels_DARK;
...
if isfield(datosegm,'umbral_npixelsmax')
    umbral_npixelsmax_LIGHT = datosegm.umbral_npixelsmax_LIGHT;
    umbral_npixelsmax_DARK = datosegm.umbral_npixelsmax_DARK;
else
    umbral_npixelsmax=[];
end
...
%         frame=double(frame_orig);
        ...
        segm(c_frames).intensmed=mean(mean(frame));
        
        if segm(c_frames).intensmed >= datosegm.int_thresh
            
            umbral_npixels = umbral_npixels_LIGHT;
            umbral_npixelsmax = umbral_npixelsmax_LIGHT;
            umbral_orig = datosegm.umbral_LIGHT;
            
        elseif segm(c_frames).intensmed < datosegm.int_thresh
            
            umbral_npixels = umbral_npixels_DARK;
            umbral_npixelsmax = umbral_npixelsmax_DARK;
            umbral_orig = datosegm.umbral_DARK;
        end
%         fprintf('\numbral set to %.2f\n',umbral_orig);
...
%         keyboard
        ...
        if limpiamierda
            ...
            % ****************** ATP CHANGED NEXT LINE ********************
            buenos=tams>umbral_npixels & tams<umbral_npixelsmax; % added second part
