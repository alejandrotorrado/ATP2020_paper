...
% 08-Jan-2014 11:15:14 Hago que abra parpool aunque sólo se quiera 1
% procesador. Además no abre parpool si ya está abierto con el número de
...
% Además, hago que no abra parpool si está en Trueno
...
%         fprintf(datosegm.id_log,'%s - Start\n',datestr(now,30));
        ...
        if ~isfield(datosegm,'saltatodo') || ~datosegm.saltatodo
            ...
            %     end
            %     if ispc && parpool('size')==0 % Sólo abre parpool si está en un pc, porque si es linux indica que estamos en Trueno.
            % %                 disp('Guarning! No se abre parpool!')
            %         parpool open % Lo abre con la configuración por defecto par el ordenador
            %         %         disp('Guarning! parpool se abre sólo con 6 procesadores')
...
%                     parpool open % Configuración por defecto del ordenador
%                 else
