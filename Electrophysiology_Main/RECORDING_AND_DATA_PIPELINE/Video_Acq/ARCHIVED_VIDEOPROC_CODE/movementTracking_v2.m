...
while ~isDone(vidSrc)
    ...
    % take 1 frame every 200
    while mod(counter0,2000) ~= 0 && ~isDone(vidSrc)
        counter0 = counter0 + 1;
        frame = step(vidSrc);
    end
    
    % Get average intensity of frame
    avgIntensity(end+1,1) = mean(mean(frame));
    
    if mod(counter0,1000)==0
