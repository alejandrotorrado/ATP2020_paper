...
chunk_sz = 2000;
nchunks = ceil(nvid_frames/chunk_sz);

vidSrc = VideoReader(videofilename);
% profile on
t0=tic;
for chunk = 1:nchunks
    startpos = (chunk-1)*chunk_sz+1;
    endpos   = min(chunk*chunk_sz,nvid_frames);
    ...
    toc
    % have all the mvt data from one chunk, smooth it, store it.
end
