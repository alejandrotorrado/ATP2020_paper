...
rawVideoDir = uigetdir(cd,'video dir');
saveDir = uigetdir(cd,'save dir');
animal = 'KH65';
framerate = 30;

vidTimestamps = [saveDir filesep animal '_frameTimes.mat'];
framefiles = dir([rawVideoDir filesep 'Frametimes_*.mat']);
...
rawfiles = dir([rawVideoDir filesep '*.avi']);
...
for ii = 1:N_files
    ...
    for ee = 1:size(images,2)
        ...
        if ~isempty(img)
            frametimes(filecounter,1) = times(ee);
