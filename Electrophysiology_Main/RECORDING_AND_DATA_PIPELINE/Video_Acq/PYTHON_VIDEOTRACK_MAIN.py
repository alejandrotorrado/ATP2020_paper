...
if sys.platform == 'win32':
    ...
    sys.path.insert(0, 'Z:\RECORDING_AND_DATA_PIPELINE\Video_Acq')
elif sys.platform == 'darwin':
    print('Running on Mac OS.')
    sys.path.insert(0, '/Volumes/turrigiano-lab/RECORDING_AND_DATA_PIPELINE/Video_Acq')
...
animal = raw_input('Animal name? (e.g. AT16) ')
recname = raw_input('Recording name? (e.g. BL03_04) ')

# create video capture object pointing to video
if sys.platform == 'win32':
    vidFile = vidFile.encode('utf-8')
    vidFile = vidFile.replace("/","\\")
