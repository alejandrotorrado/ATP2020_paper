...
isOkAnim = 0
while isOkAnim == 0:
	animal = input('Input animal name (e.g. KH65_66):  ')
	print('You chose {} as the animal name'.format(animal))
	isOkAnim = int(input('Is this correct? (1=yes,0=no):  '),2)
...
frate = 10. # this is the number of frames collected per second

# files will turn over/save every nmin minutes
nmin        = 2 # how many minutes you store in each file
ncycles     = nmin*60*frate

# Deal with the Metadata
piDir = "//129.64.111.15/ExpLogs_B"
checkerDir = piDir + '/RECORD'
try:
	os.stat(checkerDir)
	...
	cmd_string = r'net use K:\ \\129.64.111.15\ExpLogs_B /USER pi invivoB'
...
macminichecker = os.path.join(checkerDir,'rigb_vidstat.bin')
try:
...
while True:
	...
	r = 704.0 / frame.shape[1]    
	

	#dim = (704, int(frame.shape[0]*r))
	dim  = (704,576)
