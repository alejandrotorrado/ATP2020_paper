...
%
%                       THIS ONE ADDED BY ATP 12/14/2017
%                       +'BoxType' - String, either 'SEM', 'CI' or 'Quart'.
%                       'SEM' makes the plot show a box for the SEM around
%                       the mean. 'CI' makes it show a 95% CI based on the
%                       calculated SEM (this was the default). 'Quart'
%                       makes it show inter-quartile range (as in a
%                       standard box plot). Default is 'SEM'.
%
%                       THIS ONE ADDED BY ATP 1/10/2018
%                       + 'DataTransform' - string, for now 'Log' is only
%                       accepted value. This tells the function the data
%                       have been transformed. The mean will then be
%                       calculated on the untransformed data, so that the
%                       plot always shows the arithmetic mean of the data.
%
...
%% THIS ADDED BY ATP 12/14/2017
Ind=strmatch('BoxType',stringVars);
if ~isempty(Ind)
    %Check that PointStyle is a string
    if ischar(valueVars{Ind}) 
        BoxType=valueVars{Ind};
    else
        error('wrong SEMorCI, it should be a string (''SEM'' or ''CI'') determining whether to show SEM or 95%% CI in plot.')
    end
else
    BoxType = 'SEM';
end

Ind=strmatch('DataTransform',stringVars);
if ~isempty(Ind)
    %Check that PointStyle is a string
    if ischar(valueVars{Ind}) 
        DataTransform=valueVars{Ind};
    else
        error('wrong SEMorCI, it should be a string (''SEM'' or ''CI'') determining whether to show SEM or 95%% CI in plot.')
    end
else
...
Ind=strmatch('X_VALS',stringVars);
if ~isempty(Ind)
    %Check that X_VALS is a number or array of numbers
    if isnumeric(valueVars{Ind}) && all(size(valueVars{Ind},1)== 1)
        X_VALS=valueVars{Ind};
    else
        error(sprintf('wrong X_VALS, it should be a row vector of numbers'))
    end
else
   X_VALS = 1:1:size(data,2);
end
...
yPositions=data;
for i=1:size(data,2)
%% this added by ATP for custom x positions of box plots
ix = X_VALS(i);
...
% overlap.
% NOTE: ATP changed this to ix instead of i. 12/15/17
xValues=ones(size(yValues))*ix;
...
if strcmp(Whiskers,'box')
    ...
    if strcmp(DataTransform,'None')
        yMean=mean(yValues);
        yStd=std(yValues);
        ySem=yStd/sqrt(size(yValues,1));
    elseif strcmp(DataTransform,'Log')
        keyboard;
        raw_yValues = 10.^(yValues);
        raw_yMean=mean(raw_yValues);
        raw_yStd=std(raw_yValues);
        lognorm_mean = mean(yValues);
        lognorm_std = std(yValues);
        log_mean = exp(lognorm_mean + 0.5*lognorm_std^2);
        log_var = log_mean^2 * (exp(lognorm_std^2) - 1);
        log_std = sqrt(log_var);
        yMean = log10(raw_yMean);
        
        ySem = log_std/sqrt(size(yValues,1));
    end
    if strcmp(BoxType,'CI')
        yCI=ySem*1.96;
    elseif strcmp(BoxType,'SEM')
        yCI = ySem;
    elseif strcmp(BoxType,'Quart')
        % compute inter-quartile range
%         keyboard;
        if mod(numel(yValues),2) == 0
            % if even number of data points, divide array in 2 equal parts
            % around the median
            yMedian = median(yValues);
            midval = find(yValues >= yMedian,1,'first');
            yval_Low = yValues(1:midval-1);
            yval_High = yValues(midval:end);
            % then find median of either
            q1 = median(yval_Low);
            q3 = median(yval_High);
            IQR = q3 - q1;
        elseif mod(numel(yValues),2) == 1
            yMedian = median(yValues);
            midval = find(yValues >= yMedian,1,'first');
            yval_Low = yValues(1:midval-1);
            yval_High = yValues(midval+1:end);
            % then find median of either
            q1 = median(yval_Low);
            q3 = median(yval_High);
            IQR = q3 - q1;
    ...
    rectangle('Position',[ix-Width/WhiskersWidthRatio,yMean-yStd,2*Width/WhiskersWidthRatio,2*yStd ],'FaceColor',StdColor(StdIndex,:),'EdgeColor', StdColor(StdIndex,:),'LineWidth',0.1);
    %plot the mean+-SEM box
    if any(strcmp(BoxType,{'CI','SEM'}))
%         keyboard;
        rectangle('Position',[ix-Width/WhiskersWidthRatio,yMean-yCI,2*Width/WhiskersWidthRatio,2*yCI ],'FaceColor',SEMColor(SEMIndex,:),'EdgeColor', SEMColor(SEMIndex,:),'LineWidth',0.1);
        %plot the mean line 
        plot([ix-Width/WhiskersWidthRatio ix+Width/WhiskersWidthRatio],[yMean yMean],'Color',MeanColor(MeanIndex,:),'LineWidth',3)
    elseif strcmp(BoxType,'Quart')
        rectangle('Position',[ix-Width/WhiskersWidthRatio,q1,2*Width/WhiskersWidthRatio,IQR ],'FaceColor',SEMColor(SEMIndex,:),'EdgeColor', SEMColor(SEMIndex,:),'LineWidth',0.1);
        %plot the mean line 
        plot([ix-Width/WhiskersWidthRatio ix+Width/WhiskersWidthRatio],[yMedian yMedian],'Color',MeanColor(MeanIndex,:),'LineWidth',3);
    end
...
while keep_going
    ...
    if all(sum(subsetind)~=[1 0]) %If there's only one point, we represent it in the middle, and xValues remains equal to i
        ...
        xSubset=ones(sum(subsetind),1)*ix;
...
if strcmp(Whiskers,'lines')
    %plot the mean line
    yMean=mean(yValues);
    plot([ix-Width/WhiskersWidthRatio ix+Width/WhiskersWidthRatio],[yMean yMean],'Color',MeanColor(MeanIndex),'LineWidth',1.5)
    
    %plot the sd 
    yStd=std(yValues);
    plot([ix-Width/WhiskersWidthRatio*0.5 ix+Width/WhiskersWidthRatio*0.5],[yMean+yStd yMean+yStd],'Color',StdColor(StdIndex,:),'LineWidth',1.5)
    plot([ix-Width/WhiskersWidthRatio*0.5 ix+Width/WhiskersWidthRatio*0.5],[yMean-yStd yMean-yStd],'Color',StdColor(StdIndex,:),'LineWidth',1.5)
    plot([ix ix],[yMean-yStd yMean+yStd],'Color',StdColor(StdIndex,:))
    %plot the conf. interval of the mean
    ySem=yStd/sqrt(size(yValues,1));
    yCI=ySem*1.96;
    plot([ix-Width/WhiskersWidthRatio*0.7 ix+Width/WhiskersWidthRatio*0.7],[yMean+yCI yMean+yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',1.5)
    plot([ix-Width/WhiskersWidthRatio*0.7 ix+Width/WhiskersWidthRatio*0.7],[yMean-yCI yMean-yCI],'Color',SEMColor(SEMIndex,:),'LineWidth',1.5)
    plot([ix ix],[yMean-yCI yMean+yCI],'Color',SEMColor(SEMIndex,:))
