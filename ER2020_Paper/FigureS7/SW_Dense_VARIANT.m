%% S/W-DENSE ANALYSIS - SW_Dense_VARIANT.m
%
% Alejandro Torrado Pacheco - 2017
%
% ____________________________ IMPORTANT NOTE ____________________________
%
% This variant of the code is used to create the data to make Fig S7. It
% loops through several 'dense' threshold values with a fixed window size
% to create each panel in Fig S7. To create a different panel, change the
% window size. The step size can also be changed (not plotted in Fig S7).
% The main version of this script, used to reproduce Fig 4, is called
% SW_Dense_MAIN.m
...
% *** NOTE: change the window_hours parameter to reproduce different panels
% of Fig S7. For Fig S7, window_step and FR_time_window should both be 15
% minutes. These can be varied if you want to see how changing them affects
...
% 'dense' threshold values to loop through. For Fig S7, this array should
% include 3 elements: 0.70, 0.75, 0.80. You can add more to check how that
% affects the analysis results.
thresh_to_test = [0.70, 0.75, 0.80];

%% loop through threshold values
% this loop goes around the loops through animals, states and cells
for tt = 1:length(thresh_to_test)
    ...
    for aa = 1:numel(all_animals)
        ...
        if any(strcmp(animal,which_anim)) && any(strcmp(animal,anims))
            ...
            t_dense_thresh = thresh_to_test(tt);
    ...
    if save_the_data
        ...
        % start and end of S/W-dense epochs
        ...
        if tt == numel(thresh_to_test)
            save(save_file,'swdense_data','-v7.3');
        end
    end % end if save_data loop
end % thresh_test loop
