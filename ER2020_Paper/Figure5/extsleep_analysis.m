...
sleep_starts_nonan = cat_sleep_starts(delta_nonans);
sleep_starts_nonan_REM = cat_sleep_starts(rem_delta_nonans);
...
if mean_t ~= 3
    ...
    sleep_starts_CIRC = mod(sleep_starts_nonan./3600, 24);
    ...
    [r_circ,p_circ] = corr(sleep_starts_CIRC, NREM_delta_nonan);
...
%     subplot(2,1,1)
    if dep
        ...
        scatter(sleep_starts_CIRC,NREM_delta_nonan,markersize^2,c_nrem);
    end
    hold on;
    if mylims
        xlims = [-1 25];
    ...
    errorbar(CIRC_group_means_offT,CIRC_NR_group_means,CIRC_NR_group_sem,'o','capsize',12,'markersize',8,...
        'linestyle','none','markerfacecolor',mfc,'linewidth',1.5,'color',c_nrem);
    hold on;
    box off
    plot(X,Y,'--k','linewidth',1.5);
...
%         text(xl2(2)*.8,yl2(2)*.9,sprintf('r = %.4f\np = %.4f',r_circ,p_circ),'fontsize',16);
    ...
    [r_circ_REM,p_circ_REM] = corr(sleep_starts_CIRC_REM, REM_delta_nonan);
    ...
    if dep
        ...
        scatter(sleep_starts_CIRC_REM,REM_delta_nonan,markersize^2,c_rem);
    end
    hold on;
    if mylims
        xlims = [-1 25];
...
%     asterisks_d = get_asterisks_from_pval(p_rho_d);
    ...
    sleep_starts_CIRC = mod(sleep_starts_nonan ./ 3600, 24);
     ...
     [rho_c,p_rho_c] = corr(sleep_starts_CIRC, NREM_delta_nonan);
%     asterisks_d = get_asterisks_from_pval(p_rho_d);
