...
for aa = 1:n_anims
    ...
    triplets = find_sleep_triplets(statetimes);
...
%         keyboard;
        n_RSU = size(rsu,2);
        fprintf('Found %u cells for this animal.\n',n_RSU);
        ...
        if n_RSU > 0 && do_power
            ...
            fprintf('Skip LFP load since we don''t need it.\n');
        end
...
%                             if sum(rate_norm > big_val_thresh) / size(rate_norm,1) >= ceil(1e2/60)
%                                 disp('toobig');
%                                 toobig_ = toobig_ + 1;
%                                 rate_tmp(:) = NaN;
%                             end
%                             
%                             if sum(rate_norm < small_val_thresh) / size(rate_norm,1) >= ceil(1e4/60)
%                                 disp('toosmall');
%                                 toosmall_ = toosmall_ + 1;
%                                 rate_tmp(:) = NaN;
%                             end
                            ...
                            if ~isnan(rate_tmp)
                                ...
                                if sum(rate_long==0) > 0 %numel(rate_long) / 4
                                    ...
                                    if do_packets
                                        ...
                                        NR1_packet_1_idx0 = ceil( (NR1_t0 + min_end_excl - mean_long_t0) / G_bin);
                                        NR1_packet_1_idx1 = floor( (NR1_t0 + min_end_excl + min_fr_len - mean_long_t0) / G_bin);
                                        if NR1_packet_1_idx1 > NR1_idx1
                                            NR1_packet_1 = NaN;
                                            NR1_packet_2 = NaN;
                                        else
                                            NR1_packet_2 = NR1_packet_FR;
                                            NR1_packet_1 = nanmean(rate_long(NR1_packet_1_idx0:NR1_packet_1_idx1));
                                        end
                                        % NR2
                                        NR2_packet_2_idx1 = ceil( (NR2_t1 - min_end_excl - mean_long_t0) / G_bin);
                                        NR2_packet_2_idx0 = floor( (NR2_t1 - min_end_excl - min_fr_len - mean_long_t0) / G_bin);
                                        if NR2_packet_2_idx0 < NR2_idx0
                                            NR2_packet_1 = NaN;
                                            NR2_packet_2 = NaN;
                                        else
                                            NR2_packet_1 = NR2_packet_FR;
                                            NR2_packet_2 = nanmean(rate_long(NR2_packet_2_idx0:NR2_packet_2_idx1));
                                        end
                                        % REM
                                        if REM_t1 - REM_t0 >= REM_dur_thresh
                                            REM_packet_1_idx0 = ceil( (REM_t0 + min_end_excl_REM - mean_long_t0) / G_bin);
                                            REM_packet_1_idx1 = floor( (REM_t0 + min_end_excl_REM + min_fr_len_REM - mean_long_t0) / G_bin);
                                            REM_packet_2_idx0 = floor( (REM_t1 - min_end_excl_REM - min_fr_len_REM - mean_long_t0) / G_bin);
                                            REM_packet_2_idx1 = ceil( (REM_t1 - min_end_excl_REM - mean_long_t0) / G_bin);
...
if save_the_data
    ...
    if ispc
        save_dir = 'Z:\ATP_MAIN\CODE\lfp_analysis_beta\Analysis_Data';
    elseif ismac
        save_dir = '/Volumes/turrigiano-lab/ATP_MAIN/CODE/lfp_analysis_beta/Analysis_Data';
    end
    dep_str = {'CTRL','DEP'};
    save_file = ['triplet_data_v' num2str(ID_NUM) '_' dep_str{dep+1} '.mat'];
